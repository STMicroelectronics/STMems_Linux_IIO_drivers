...
static irqreturn_t st_mag3d_trigger_handler_thread(int irq, void *p)
{
	...
	iio_trigger_poll_chained(hw->trig, hw->timestamp);
...
int st_mag3d_allocate_trigger(struct iio_dev *iio_dev)
{
	...
	hw->trig = iio_trigger_alloc("%s-trigger", iio_dev->name);
	if (!hw->trig)
		return -ENOMEM;
	...
	err = iio_trigger_register(hw->trig);
	if (err < 0)
		iio_trigger_free(hw->trig);
...
static int st_mag3d_buffer_preenable(struct iio_dev *iio_dev)
{
	...
	err = st_mag3d_enable_sensor(iio_priv(iio_dev), true);
	if (err < 0)
		return err;
