...
static inline s64 st_stts22h_get_time_ns(void)
{
...
enum hrtimer_restart st_stts22h_poll_function_read(struct hrtimer *timer)
{
	...
	data->timestamp = st_stts22h_get_time_ns();
...
static void st_stts22h_poll_function_work(struct work_struct *iio_work)
{
	...
	/* adjust new timeout */
	ktdelta = ktime_set(0, (st_stts22h_get_time_ns() - data->timestamp));
			    ...
			    const struct i2c_device_id *id)
{
	...
	iio_dev = iio_device_alloc(sizeof(*data));
	if (!iio_dev)
		return -ENOMEM;
	...
	data->iio_devs->buffer = iio_kfifo_allocate(data->iio_devs);
	if (!data->iio_devs->buffer) {
		err = -ENOMEM;

		goto iio_alloc;
	}

	data->iio_devs->modes |= INDIO_BUFFER_HARDWARE;
	data->iio_devs->setup_ops = &st_stts22h_buffer_ops;

	err = iio_buffer_register(data->iio_devs,
				  data->iio_devs->channels,
				  data->iio_devs->num_channels);
	if (err)
...
buffer_allocate_error:
	iio_buffer_unregister(data->iio_devs);
	iio_kfifo_free(data->iio_devs->buffer);

iio_alloc:
	iio_device_free(data->iio_devs);
