...
#define REG_FIFO_CTRL_MODE_MASK		0xe0

#define REG_FIFO_SRC_ADDR		0x2f
#define REG_FIFO_SRC_OVR_MASK		BIT(6)
#define REG_FIFO_SRC_FSS_MASK		0x3f
...
static int st_lis3dhh_buffer_preenable(struct iio_dev *iio_dev)
{
	...
	err = st_lis3dhh_update_fifo(iio_priv(iio_dev), true);
	if (err < 0)
		return err;
...
void st_lis3dhh_deallocate_fifo(struct st_lis3dhh_hw *hw)
{
	struct iio_dev *iio_dev = iio_priv_to_dev(hw);
