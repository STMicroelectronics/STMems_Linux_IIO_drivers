...
static int st_lis2dw12_update_fifo(struct iio_dev *iio_dev, bool enable)
{
	...
	if (enable) {
		hw->ts_irq = hw->ts = st_lis2dw12_get_timestamp();
...
static int st_lis2dw12_buffer_preenable(struct iio_dev *iio_dev)
{
	...
	err = st_lis2dw12_update_fifo(iio_dev, true);
	if (err < 0)
		return err;
...
static int st_lis2dw12_read_fifo(struct st_lis2dw12_hw *hw)
{
	...
	while (read_len < fifo_len) {
		...
		for (i = 0; i < word_len; i += ST_LIS2DW12_DATA_SIZE) {
			...
			hw->ts = min_t(s64, st_lis2dw12_get_timestamp(),
				       hw->ts);
			       ...
			       const char *buf, size_t size)
{
	...
	hw->ts_irq = st_lis2dw12_get_timestamp();
...
int st_lis2dw12_fifo_setup(struct st_lis2dw12_hw *hw)
{
	...
	iio_dev->buffer = iio_kfifo_allocate(iio_dev);
	if (!iio_dev->buffer)
		return -ENOMEM;
	...
	return iio_buffer_register(iio_dev, iio_dev->channels,
				   iio_dev->num_channels);
}

int st_lis2dw12_deallocate_fifo(struct st_lis2dw12_hw *hw)
{
	iio_buffer_unregister(hw->iio_devs[ST_LIS2DW12_ID_ACC]);
	iio_kfifo_free(hw->iio_devs[ST_LIS2DW12_ID_ACC]->buffer);
