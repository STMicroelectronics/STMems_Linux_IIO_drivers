...
static int st_lis3dhh_spi_probe(struct spi_device *spi)
{
	...
	iio_dev = iio_device_alloc(sizeof(*hw));
	if (!iio_dev)
		return -ENOMEM;
	...
	err = iio_device_register(iio_dev);
	if (err < 0)
		goto deallocate_fifo;

	return 0;

deallocate_fifo:
	if (hw->irq > 0)
		st_lis3dhh_deallocate_fifo(hw);
iio_alloc_fail:
...
static int st_lis3dhh_spi_remove(struct spi_device *spi)
{
