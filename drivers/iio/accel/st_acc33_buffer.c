...
static inline s64 st_acc33_get_timestamp(void)
{
	struct timespec ts;
			      ...
			      const char *buf, size_t size)
{
	...
	hw->ts_irq = st_acc33_get_timestamp();
...
static int st_acc33_update_fifo(struct st_acc33_hw *hw, bool enable)
{
	...
	if (enable) {
		hw->ts_irq = hw->ts = st_acc33_get_timestamp();
...
static int st_acc33_buffer_preenable(struct iio_dev *iio_dev)
{
	...
	err = st_acc33_update_fifo(iio_priv(iio_dev), true);
	if (err < 0)
		return err;
...
void st_acc33_deallocate_fifo(struct st_acc33_hw *hw)
{
	struct iio_dev *iio_dev = iio_priv_to_dev(hw);
