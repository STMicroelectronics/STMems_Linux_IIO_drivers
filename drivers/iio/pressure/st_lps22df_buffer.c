...
int st_lps22df_allocate_buffers(struct st_lps22df_hw *hw)
{
	...
	/* int pin active low */
	if (np && of_property_read_bool(np, "int-active-low")) {
	...
	if (np && of_property_read_bool(np, "int-open-drain")) {
		err = st_lps22df_write_with_mask(hw, ST_LPS22DF_CTRL3_ADDR,
	...
	buffer = iio_kfifo_allocate(hw->iio_devs[ST_LPS22DF_PRESS]);
	if (!buffer) {
		err = -ENOMEM;
	...
	hw->iio_devs[ST_LPS22DF_PRESS]->modes |= INDIO_BUFFER_HARDWARE;
	hw->iio_devs[ST_LPS22DF_PRESS]->setup_ops = &st_lps22df_buffer_ops;

	err = iio_buffer_register(hw->iio_devs[ST_LPS22DF_PRESS],
				  hw->iio_devs[ST_LPS22DF_PRESS]->channels,
				  hw->iio_devs[ST_LPS22DF_PRESS]->num_channels);
	if (err)
		goto fifo_allocate_error;
...
fifo_allocate_error:
	st_lps22df_deallocate_buffers(hw);
...
{
	iio_buffer_unregister(hw->iio_devs[ST_LPS22DF_PRESS]);
	if (hw->iio_devs[ST_LPS22DF_PRESS]->buffer)
		iio_kfifo_free(hw->iio_devs[ST_LPS22DF_PRESS]->buffer);
