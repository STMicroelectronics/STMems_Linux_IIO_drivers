...
iio_register_fail:
	...
	if (hw->irq > 0)
		st_lps22hh_deallocate_buffers(hw);
iio_alloc_fail:
	for (i = 0; i < ST_LPS22HH_SENSORS_NUMB; i++) {
		if (!hw->iio_devs[i])
			continue;
...
int st_lps22hh_common_remove(struct device *dev)
{
	struct st_lps22hh_hw *hw = dev_get_drvdata(dev);
...
EXPORT_SYMBOL(st_lps22hh_common_remove);
