...
#define ST_LPS33HW_TEMP_FS_AVL_GAIN		(1000000000UL / 100UL)
...
static const struct iio_chan_spec st_lps33hw_press_channels[] = {
	...
	},
	{
		...
		.scan_index = -1,
		.indexed = -1,
		.event_mask = IIO_EV_BIT(IIO_EV_TYPE_FIFO_FLUSH,
					 IIO_EV_DIR_EITHER),
	},
...
iio_register_fail:
	...
	if (hw->irq > 0)
		st_lps33hw_deallocate_buffers(hw);
iio_alloc_fail:
	for (i = 0; i < ST_LPS33HW_SENSORS_NUMB; i++) {
		if (!hw->iio_devs[i])
			continue;
...
int st_lps33hw_common_remove(struct device *dev)
{
	struct st_lps33hw_hw *hw = dev_get_drvdata(dev);
	int i;
...
EXPORT_SYMBOL(st_lps33hw_common_remove);
