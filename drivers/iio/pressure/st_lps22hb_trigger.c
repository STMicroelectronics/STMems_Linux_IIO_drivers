...
static irqreturn_t lps22hb_irq_handler(int irq, void *private)
{
	struct lps22hb_data *cdata = private;

	cdata->prev_timestamp = cdata->sensor_timestamp;
	cdata->sensor_timestamp = lps22hb_get_time_ns();
...
static irqreturn_t lps22hb_irq_thread(int irq, void *private)
{
	struct lps22hb_data *cdata = private;
	u8 fifo_src;

	cdata->tf->read(cdata, LPS22HB_FIFO_SRC_ADDR, 1, &fifo_src);

	mutex_lock(&cdata->fifo_lock);
	if (fifo_src & LPS22HB_FIFO_SRC_FTH_MASK)
		lps22hb_flush_fifo(cdata, fifo_src);
			     ...
			     const struct iio_trigger_ops *trigger_ops)
{
	...
	err = request_threaded_irq(cdata->irq, lps22hb_irq_handler, lps22hb_irq_thread,
