...
static inline s64 st_lps22hh_get_time_ns(void)
{
	struct timespec ts;
				    ...
				    const char *buf, size_t size)
{
	...
	if (sensor->type == ST_LPS22HH_PRESS)
		...
		event = IIO_UNMOD_EVENT_CODE(IIO_TEMP, -1,
					     IIO_EV_TYPE_FIFO_FLUSH, type);
	iio_push_event(indio_dev, event, st_lps22hh_get_time_ns());
	mutex_unlock(&indio_dev->mlock);
...
{
	s64 delta_ts, ts = st_lps22hh_get_time_ns();
...
int st_lps22hh_deallocate_buffers(struct st_lps22hh_hw *hw)
{
	int i;
