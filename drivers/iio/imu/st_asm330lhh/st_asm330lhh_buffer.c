...
#define ST_ASM330LHH_REG_FIFO_LEN_MASK		0x1ff
#define ST_ASM330LHH_REG_FIFO_BATCH_ADDR	0x09
#define ST_ASM330LHH_REG_FIFO_CTRL4_ADDR	0x0a
#define ST_ASM330LHH_REG_FIFO_MODE_MASK		0x07
#define ST_ASM330LHH_REG_DEC_TS_MASK		0xc0
...
#define ST_ASM330LHH_EWMA_LEVEL			96
...
static inline int st_asm330lhh_reset_hwts(struct st_asm330lhh_hw *hw)
{
	...
	hw->ts_offset = st_asm330lhh_get_time_ns();
	hw->ts = hw->ts_offset;
						...
						bool enable)
{
	...
	return st_asm330lhh_write_with_mask(hw,
					    ST_ASM330LHH_REG_FIFO_BATCH_ADDR,
...
static void st_asm330lhh_parse_data(struct iio_dev *iio_dev, u8 *data)
{
	u8 buff[ALIGN(ST_ASM330LHH_SAMPLE_SIZE, sizeof(s64)) + sizeof(s64)];
	struct st_asm330lhh_sensor *sensor = iio_priv(iio_dev);
	struct st_asm330lhh_hw *hw = sensor->hw;
	s64 ts = hw->hw_ts + hw->ts_offset;

	memcpy(buff, &data[ST_ASM330LHH_TAG_SIZE], ST_ASM330LHH_SAMPLE_SIZE);
	iio_push_to_buffers_with_timestamp(iio_dev, buff, ts);
}

static int st_asm330lhh_read_fifo(struct st_asm330lhh_hw *hw)
{
	u8 buff[ST_ASM330LHH_FIFO_SAMPLE_SIZE], tag;
	s64 ts_irq = hw->ts - hw->delta_ts, delta;
	u16 fifo_len, fifo_depth;
	__le16 fifo_status;
	int err, read_len;
	u32 val;
	...
	for (read_len = 0; read_len < fifo_len;
	     read_len += ST_ASM330LHH_FIFO_SAMPLE_SIZE) {
		err = hw->tf->read(hw->dev, ST_ASM330LHH_REG_FIFO_OUT_TAG_ADDR,
				   ST_ASM330LHH_FIFO_SAMPLE_SIZE, buff);
		if (err < 0)
			return err;
		...
		case ST_ASM330LHH_GYRO_TAG:
			st_asm330lhh_parse_data(
					hw->iio_devs[ST_ASM330LHH_ID_GYRO],
					buff);
			break;
		case ST_ASM330LHH_ACC_TAG:
			st_asm330lhh_parse_data(
					hw->iio_devs[ST_ASM330LHH_ID_ACC],
					buff);
			break;
		case ST_ASM330LHH_TS_TAG:
			val = get_unaligned_le32(&buff[ST_ASM330LHH_TAG_SIZE]);
...
static int st_asm330lhh_buffer_preenable(struct iio_dev *iio_dev)
{
	...
	err = st_asm330lhh_update_fifo(iio_dev, true);
	if (err < 0)
		return err;
