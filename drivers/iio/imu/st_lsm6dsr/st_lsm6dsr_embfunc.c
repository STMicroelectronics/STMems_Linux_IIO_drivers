...
#define ST_LSM6DSR_REG_WR_MASK				0x60
...
#define ST_LSM6DSR_REG_FSM_ODR_MASK			0x38
...
static int st_lsm6dsr_ef_sensor_set_enable(struct st_lsm6dsr_sensor *sensor,
					   u8 mask, u8 irq_mask, bool enable)
{
	...
	err = __st_lsm6dsr_write_with_mask(hw, ST_LSM6DSR_REG_EMB_FUNC_EN_A_ADDR,
					   mask, enable);
	if (err < 0)
		goto reset_page;
					 ...
					 bool enable)
{
	...
	case ST_LSM6DSR_ID_STEP_DETECTOR:
		err = st_lsm6dsr_ef_sensor_set_enable(sensor,
...
{
	u8 nfsm[] = { ARRAY_SIZE(st_lsm6dsr_fsm_sensor_list),
	...
	err  = __st_lsm6dsr_write_with_mask(hw,
					    ST_LSM6DSR_PAGE_RW_ADDR,
					    ST_LSM6DSR_REG_EMB_FUNC_LIR_MASK, 1);
	if (err < 0)
		goto reset_page;
