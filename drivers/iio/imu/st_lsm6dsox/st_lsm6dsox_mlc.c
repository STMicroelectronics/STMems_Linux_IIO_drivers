...
static int st_lsm6dsox_mlc_flush_all(struct st_lsm6dsox_hw *hw, bool remove)
{
	struct st_lsm6dsox_sensor *sensor_mlc;
	struct iio_dev *iio_dev;
	int ret = 0, id;
	int start = ST_LSM6DSOX_ID_MLC_0;

	if (remove)
		start = ST_LSM6DSOX_ID_MLC;

	for (id = start; id < ST_LSM6DSOX_ID_MAX; id++) {
				     ...
				     const char *buf, size_t size)
{
	...
	ret = st_lsm6dsox_mlc_flush_all(hw, 0);
					      ...
					      enum st_lsm6dsox_sensor_id id)
{
	...
	case ST_LSM6DSOX_ID_MLC: {
		const struct iio_chan_spec st_lsm6dsox_mlc_channels[] = {
			ST_LSM6DSOX_EVENT_CHANNEL(IIO_ACTIVITY,
						  IIO_EV_TYPE_THRESH,
						  IIO_EV_DIR_RISING),
		};
...
int st_lsm6dsox_mlc_remove(struct device *dev)
{
	...
	return st_lsm6dsox_mlc_flush_all(hw, 1);
}
