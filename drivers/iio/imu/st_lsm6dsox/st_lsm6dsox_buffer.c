...
{
	u8 iio_buf[ALIGN(ST_LSM6DSOX_SAMPLE_SIZE, sizeof(s64)) + sizeof(s64)];
...
static irqreturn_t st_lsm6dsox_handler_irq(int irq, void *private)
{
	...
	s64 ts = st_lsm6dsox_get_time_ns();
...
int st_lsm6dsox_buffers_setup(struct st_lsm6dsox_hw *hw)
{
	...
	err = regmap_update_bits(hw->regmap,
				 ...
				 ST_LSM6DSOX_SHIFT_VAL(irq_active_low,
				  ST_LSM6DSOX_REG_H_LACTIVE_MASK));
	if (err < 0)
		return err;
	...
	err = regmap_update_bits(hw->regmap, ST_LSM6DSOX_REG_FIFO_CTRL4_ADDR,
