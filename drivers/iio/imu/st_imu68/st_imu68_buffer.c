...
static irqreturn_t st_imu68_trigger_thread_handler(int irq, void *p)
{
	...
	for (i = 0; i < ST_IMU68_ID_MAX; i++) {
		...
		if (status & sensor->status_mask) {
			iio_trigger_poll_chained(sensor->trigger, 0);
			count++;
		}
	}
...
int st_imu68_allocate_triggers(struct st_imu68_hw *hw)
{
	...
	for (i = 0; i < ST_IMU68_ID_MAX; i++) {
		sensor = iio_priv(hw->iio_devs[i]);
		sensor->trigger = iio_trigger_alloc("%s-trigger",
						    hw->iio_devs[i]->name);
		if (!sensor->trigger) {
...
err:
	...
	for (i = 0; i < ST_IMU68_ID_MAX; i++) {
		sensor = iio_priv(hw->iio_devs[i]);
		if (sensor->trigger)
			iio_trigger_free(sensor->trigger);
	}
...
static int st_imu68_buffer_preenable(struct iio_dev *iio_dev)
{
	...
	err = st_imu68_sensor_enable(iio_priv(iio_dev), true);
	if (err < 0)
		return err;
