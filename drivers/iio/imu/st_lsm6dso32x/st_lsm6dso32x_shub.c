/*
 * STMicroelectronics st_lsm6dso32x sensor hub library driver
 *
...
#define ST_LSM6DSO32X_MAX_SLV_NUM		2

/**
 * @struct  st_lsm6dso32x_ext_pwr
...
/**
 * @struct  st_lsm6dso32x_ext_dev_settings
...
struct st_lsm6dso32x_ext_dev_settings {
	...
	u8 wai_addr;
	u8 wai_val;
	struct st_lsm6dso32x_odr_table_entry odr_table;
	struct st_lsm6dso32x_fs_table_entry fs_table;
	struct st_lsm6dso32x_reg temp_comp_reg;
	struct st_lsm6dso32x_ext_pwr pwr_table;
	struct st_lsm6dso32x_reg off_canc_reg;
	struct st_lsm6dso32x_reg bdu_reg;
...
static const struct st_lsm6dso32x_ext_dev_settings st_lsm6dso32x_ext_dev_table[] = {
	{
		/* LIS2MDL */
		.i2c_addr = { 0x1e },
		.wai_addr = 0x4f,
		...
		.ext_channels[0] = ST_LSM6DSO32X_DATA_CHANNEL(IIO_MAGN, 0x68,
							    1, IIO_MOD_X, 0,
							    16, 16, 's'),
		.ext_channels[1] = ST_LSM6DSO32X_DATA_CHANNEL(IIO_MAGN, 0x6a,
							    1, IIO_MOD_Y, 1,
							    16, 16, 's'),
		.ext_channels[2] = ST_LSM6DSO32X_DATA_CHANNEL(IIO_MAGN, 0x6c,
							    1, IIO_MOD_Z, 2,
							    16, 16, 's'),
		.ext_channels[3] = ST_LSM6DSO32X_EVENT_CHANNEL(IIO_MAGN, flush),
	...
	},
	{
		...
		.ext_available_scan_masks = { 0x7, 0x0 },
		.ext_channels[0] = ST_LSM6DSO32X_DATA_CHANNEL(IIO_MAGN, 0x28,
							    1, IIO_MOD_X, 0,
							    16, 16, 's'),
		.ext_channels[1] = ST_LSM6DSO32X_DATA_CHANNEL(IIO_MAGN, 0x2a,
							    1, IIO_MOD_Y, 1,
							    16, 16, 's'),
		.ext_channels[2] = ST_LSM6DSO32X_DATA_CHANNEL(IIO_MAGN, 0x2c,
							    1, IIO_MOD_Z, 2,
							    16, 16, 's'),
		.ext_channels[3] = ST_LSM6DSO32X_EVENT_CHANNEL(IIO_MAGN, flush),
		.ext_channels[4] = IIO_CHAN_SOFT_TIMESTAMP(3),
	...
	},
	{
		...
		.ext_channels[0] = ST_LSM6DSO32X_DATA_CHANNEL(IIO_PRESSURE, 0x28,
							    0, IIO_NO_MOD, 0,
							    24, 32, 'u'),
		.ext_channels[1] = ST_LSM6DSO32X_EVENT_CHANNEL(IIO_PRESSURE,
							     flush),
 ...
 */
static inline void st_lsm6dso32x_shub_wait_complete(struct st_lsm6dso32x_hw *hw)
{
	struct st_lsm6dso32x_sensor *sensor;
	int odr, uodr;

	sensor = iio_priv(hw->iio_devs[ST_LSM6DSO32X_ID_ACC]);
 ...
 */
static int st_lsm6dso32x_shub_read_reg(struct st_lsm6dso32x_hw *hw, u8 addr,
				       u8 *data, int len)
{
	int err;
	...
	err = st_lsm6dso32x_set_page_access(hw, true,
					    ST_LSM6DSO32X_REG_SHUB_REG_MASK);
	if (err < 0)
		goto out;
 ...
 * @return  0 if OK, < 0 if ERROR
 */
static int st_lsm6dso32x_shub_write_reg(struct st_lsm6dso32x_hw *hw, u8 addr,
					u8 *data, int len)
{
	int err;
 ...
 * @return  0 if OK, < 0 if ERROR
 */
static int st_lsm6dso32x_shub_master_enable(struct st_lsm6dso32x_sensor *sensor,
					    bool enable)
{
	...
	err = __st_lsm6dso32x_write_with_mask(hw,
				ST_LSM6DSO32X_REG_MASTER_CONFIG_ADDR,
				ST_LSM6DSO32X_REG_MASTER_ON_MASK,
				enable);
 ...
 * @return  0 if OK, < 0 if ERROR
 */
static int st_lsm6dso32x_shub_read(struct st_lsm6dso32x_sensor *sensor,
				   u8 addr, u8 *data, int len)
{
	struct st_lsm6dso32x_ext_dev_info *ext_info = &sensor->ext_dev_info;
	struct st_lsm6dso32x_hw *hw = sensor->hw;
	u8 out_addr = ST_LSM6DSO32X_REG_SLV0_OUT_ADDR + hw->ext_data_len;
	u8 config[3];
	int err;
	...
	err = st_lsm6dso32x_shub_write_reg(hw, ST_LSM6DSO32X_REG_SLV3_ADDR,
					   config, sizeof(config));
	if (err < 0)
		return err;

	err = st_lsm6dso32x_shub_master_enable(sensor, true);
	if (err < 0)
		return err;

	st_lsm6dso32x_shub_wait_complete(hw);

	err = st_lsm6dso32x_shub_read_reg(hw, out_addr, data, len & 0x7);

	st_lsm6dso32x_shub_master_enable(sensor, false);

	memset(config, 0, sizeof(config));

	return st_lsm6dso32x_shub_write_reg(hw, ST_LSM6DSO32X_REG_SLV3_ADDR,
					    config, sizeof(config));
}
 ...
 * @return  0 if OK, < 0 if ERROR
 */
static int st_lsm6dso32x_shub_write(struct st_lsm6dso32x_sensor *sensor,
				    u8 addr, u8 *data, int len)
{
	struct st_lsm6dso32x_ext_dev_info *ext_info = &sensor->ext_dev_info;
	struct st_lsm6dso32x_hw *hw = sensor->hw;
	u8 mconfig = ST_LSM6DSO32X_REG_WRITE_ONCE_MASK | 3 | hw->i2c_master_pu;
	u8 config[3] = {};
	int err, i;

	/* AuxSens = 3 + wr once + pull up configuration */
	err = st_lsm6dso32x_shub_write_reg(hw,
				ST_LSM6DSO32X_REG_MASTER_CONFIG_ADDR,
	...
	for (i = 0; i < len; i++) {
		...
		err = st_lsm6dso32x_shub_write_reg(hw,
						   ST_LSM6DSO32X_REG_SLV0_ADDR,
						   config, sizeof(config));
		if (err < 0)
			return err;

		err = st_lsm6dso32x_shub_write_reg(hw,
					ST_LSM6DSO32X_REG_DATAWRITE_SLV0_ADDR,
					&data[i], 1);
		if (err < 0)
			return err;
	...
	return st_lsm6dso32x_shub_write_reg(hw, ST_LSM6DSO32X_REG_SLV0_ADDR,
					    config, sizeof(config));
}
 ...
 * @return  0 if OK, < 0 if ERROR
 */
static int st_lsm6dso32x_shub_write_with_mask(struct st_lsm6dso32x_sensor *sensor,
					      u8 addr, u8 mask, u8 val)
{
	int err;
	u8 data;

	err = st_lsm6dso32x_shub_read(sensor, addr, &data, sizeof(data));
	if (err < 0)
		return err;

	data = (data & ~mask) | ST_LSM6DSO32X_SHIFT_VAL(val, mask);

	return st_lsm6dso32x_shub_write(sensor, addr, &data, sizeof(data));
}
 ...
 * @return  0 if OK, < 0 if ERROR
 */
static int st_lsm6dso32x_shub_config_channels(struct st_lsm6dso32x_sensor *sensor,
					      bool enable)
{
	struct st_lsm6dso32x_ext_dev_info *ext_info;
	struct st_lsm6dso32x_hw *hw = sensor->hw;
	...
	for (i = ST_LSM6DSO32X_ID_EXT0; i <= ST_LSM6DSO32X_ID_EXT1; i++) {
		if (!hw->iio_devs[i])
			continue;
		...
		config[j + 2] = ST_LSM6DSO32X_REG_BATCH_EXT_SENS_EN_MASK |
				(ext_info->ext_dev_settings->data_len &
				 ST_LSM6DSO32X_REG_SLAVE_NUMOP_MASK);
		j += 3;
	}

	return st_lsm6dso32x_shub_write_reg(hw, ST_LSM6DSO32X_REG_SLV1_ADDR,
					    config, sizeof(config));
}
 ...
 */
static int st_lsm6dso32x_shub_get_odr_val(struct st_lsm6dso32x_sensor *sensor,
					  u16 odr, u8 *val)
{
	struct st_lsm6dso32x_ext_dev_info *ext_info = &sensor->ext_dev_info;
	int i;
 ...
 * @return  0 if OK, negative value for ERROR
 */
static int st_lsm6dso32x_shub_set_odr(struct st_lsm6dso32x_sensor *sensor,
				      u16 odr)
{
	struct st_lsm6dso32x_ext_dev_info *ext_info = &sensor->ext_dev_info;
	struct st_lsm6dso32x_hw *hw = sensor->hw;
	...
	err = st_lsm6dso32x_shub_get_odr_val(sensor, odr, &odr_val);
	if (err < 0)
		return err;
	...
	return st_lsm6dso32x_shub_write_with_mask(sensor,
				  ...
				  bool enable)
{
	...
	err = st_lsm6dso32x_shub_config_channels(sensor, enable);
	if (err < 0)
		return err;

	if (enable) {
		err = st_lsm6dso32x_shub_set_odr(sensor, sensor->odr);
		if (err < 0)
			return err;
	} else {
		err = st_lsm6dso32x_shub_write_with_mask(sensor,
				ext_info->ext_dev_settings->odr_table.reg.addr,
				ext_info->ext_dev_settings->odr_table.reg.mask,
				0);
		if (err < 0)
	...
	return st_lsm6dso32x_shub_master_enable(sensor, enable);
}

static inline u32 st_lsm6dso32x_get_unaligned_le24(const u8 *p)
{
 ...
 */
static int st_lsm6dso32x_shub_read_oneshot(struct st_lsm6dso32x_sensor *sensor,
					   ...
					   int *val)
{
	...
	err = st_lsm6dso32x_shub_set_enable(sensor, true);
	if (err < 0)
		return err;
	...
	err = st_lsm6dso32x_shub_read(sensor, ch->address, data, len);
	if (err < 0)
		return err;

	st_lsm6dso32x_shub_set_enable(sensor, false);

	switch (len) {
	case 3:
		*val = (s32)st_lsm6dso32x_get_unaligned_le24(data);
		break;
	case 2:
 ...
 */
static int st_lsm6dso32x_shub_read_raw(struct iio_dev *iio_dev,
				       struct iio_chan_spec const *ch,
				       int *val, int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		...
		ret = st_lsm6dso32x_shub_read_oneshot(sensor, ch, val);
 ...
 * @return  0 if OK, -EINVAL value for ERROR
 */
static int st_lsm6dso32x_shub_write_raw(struct iio_dev *iio_dev,
					...
					int val, int val2, long mask)
{
	...
	case IIO_CHAN_INFO_SAMP_FREQ: {
		...
		err = st_lsm6dso32x_shub_get_odr_val(sensor, val, &data);
		if (!err)
			sensor->odr = val;
		break;
	}
 ...
 */
static ssize_t
st_lsm6dso32x_sysfs_shub_sampling_freq_avail(struct device *dev,
 ...
 * @return  buffer len
 */
static ssize_t st_lsm6dso32x_sysfs_shub_scale_avail(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct st_lsm6dso32x_sensor *sensor = iio_priv(dev_get_drvdata(dev));
...
static IIO_DEV_ATTR_SAMP_FREQ_AVAIL(st_lsm6dso32x_sysfs_shub_sampling_freq_avail);
static IIO_DEVICE_ATTR(in_ext_scale_available, 0444,
		       st_lsm6dso32x_sysfs_shub_scale_avail, NULL, 0);
...
static struct attribute *st_lsm6dso32x_ext_attributes[] = {
...
static const struct attribute_group st_lsm6dso32x_ext_attribute_group = {
	.attrs = st_lsm6dso32x_ext_attributes,
};

static const struct iio_info st_lsm6dso32x_ext_info = {
	.driver_module = THIS_MODULE,
	.attrs = &st_lsm6dso32x_ext_attribute_group,
	.read_raw = st_lsm6dso32x_shub_read_raw,
	.write_raw = st_lsm6dso32x_shub_write_raw,
};
 ...
 */
static struct iio_dev *st_lsm6dso32x_shub_alloc_iio_dev(struct st_lsm6dso32x_hw *hw,
			const struct st_lsm6dso32x_ext_dev_settings *ext_settings,
			enum st_lsm6dso32x_sensor_id id, u8 i2c_addr)
{
	struct st_lsm6dso32x_sensor *sensor;
	...
	iio_dev->info = &st_lsm6dso32x_ext_info;
	iio_dev->channels = ext_settings->ext_channels;
	iio_dev->num_channels = ext_settings->ext_chan_depth;

	switch (iio_dev->channels[0].type) {
	case IIO_MAGN:
		iio_dev->name = ST_LSM6DSO32X_DEV_NAME "_magn";
		break;
	case IIO_PRESSURE:
		iio_dev->name = ST_LSM6DSO32X_DEV_NAME "_press";
		break;
	default:
		iio_dev->name = ST_LSM6DSO32X_DEV_NAME "_ext";
		break;
	}
...
static int st_lsm6dso32x_shub_init_remote_sensor(struct st_lsm6dso32x_sensor *sensor)
{
	struct st_lsm6dso32x_ext_dev_info *ext_info = &sensor->ext_dev_info;
	int err = 0;
...
{
	const struct st_lsm6dso32x_ext_dev_settings *settings;
	struct st_lsm6dso32x_sensor *acc_sensor, *sensor;
	...
	if (np && of_property_read_bool(np, "drive-pullup-shub")) {
		...
		err = st_lsm6dso32x_shub_read_reg(hw,
					ST_LSM6DSO32X_REG_MASTER_CONFIG_ADDR,
					&data, sizeof(data));
		if (err < 0)
			return err;

		data |= ST_LSM6DSO32X_REG_SHUB_PU_EN_MASK;
		err = st_lsm6dso32x_shub_write_reg(hw,
					ST_LSM6DSO32X_REG_MASTER_CONFIG_ADDR,
					&data, sizeof(data));
		...
		hw->i2c_master_pu = ST_LSM6DSO32X_REG_SHUB_PU_EN_MASK;
	}

	acc_sensor = iio_priv(hw->iio_devs[ST_LSM6DSO32X_ID_ACC]);
	while (i < ARRAY_SIZE(st_lsm6dso32x_ext_dev_table) &&
	       num_ext_dev < ST_LSM6DSO32X_MAX_SLV_NUM) {
		settings = &st_lsm6dso32x_ext_dev_table[i];
		...
		for (j = 0; j < ARRAY_SIZE(settings->i2c_addr); j++) {
			...
			err = st_lsm6dso32x_shub_master_enable(acc_sensor,
							       true);
			if (err < 0)
				return err;
			...
			err = st_lsm6dso32x_shub_read_reg(hw,
						ST_LSM6DSO32X_REG_SLV0_OUT_ADDR,
						&data, sizeof(data));

			st_lsm6dso32x_shub_master_enable(acc_sensor, false);
			...
			id = ST_LSM6DSO32X_ID_EXT0 + num_ext_dev;
			hw->iio_devs[id] = st_lsm6dso32x_shub_alloc_iio_dev(hw,
							settings, id,
							settings->i2c_addr[j]);
			if (!hw->iio_devs[id])
				return -ENOMEM;
			...
			err = st_lsm6dso32x_shub_init_remote_sensor(sensor);
			if (err < 0)
				return err;
	...
	memset(config, 0, sizeof(config));
	err = st_lsm6dso32x_shub_write_reg(hw, ST_LSM6DSO32X_REG_SLV0_ADDR,
					   config, sizeof(config));
	if (err < 0)
		return err;

	/* AuxSens = 3 + wr once */
	data = ST_LSM6DSO32X_REG_WRITE_ONCE_MASK | 3 | hw->i2c_master_pu;
	return st_lsm6dso32x_shub_write_reg(hw,
				ST_LSM6DSO32X_REG_MASTER_CONFIG_ADDR,
				&data, sizeof(data));
}
