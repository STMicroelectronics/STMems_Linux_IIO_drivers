...
static const struct st_asm330lhhx_ext_dev_settings st_asm330lhhx_ext_dev_table[] = {
	{
		...
		.ext_channels[2] = ST_ASM330LHHX_DATA_CHANNEL(IIO_MAGN, 0x6c,
							    1, IIO_MOD_Z, 2,
							    16, 16, 's'),
		.ext_channels[3] = ST_ASM330LHHX_EVENT_CHANNEL(IIO_MAGN,
					  ...
					  bool enable)
{
	...
	st_asm330lhhx_set_page_access(hw, false, ST_ASM330LHHX_REG_SHUB_REG_MASK);
 ...
 * @return  0 if OK, < 0 if ERROR
 */
static int st_asm330lhhx_shub_write(struct st_asm330lhhx_sensor *sensor, u8 addr,
				  u8 *data, int len)
{
	struct st_asm330lhhx_ext_dev_info *ext_info = &sensor->ext_dev_info;
	struct st_asm330lhhx_hw *hw = sensor->hw;
 ...
 * @return  0 if OK, negative value for ERROR
 */
static int st_asm330lhhx_shub_set_odr(struct st_asm330lhhx_sensor *sensor, u16 odr)
{
	struct st_asm330lhhx_ext_dev_info *ext_info = &sensor->ext_dev_info;
	struct st_asm330lhhx_hw *hw = sensor->hw;
	u8 odr_val;
 ...
 * @return  0 if OK, negative value for ERROR
 */
int st_asm330lhhx_shub_set_enable(struct st_asm330lhhx_sensor *sensor, bool enable)
{
	struct st_asm330lhhx_ext_dev_info *ext_info = &sensor->ext_dev_info;
	int err;
 ...
 * @return  0 if OK, -EINVAL value for ERROR
 */
static int st_asm330lhhx_shub_write_raw(struct iio_dev *iio_dev,
			...
			enum st_asm330lhhx_sensor_id id, u8 i2c_addr)
{
	...
	case IIO_MAGN:
		iio_dev->name = ST_ASM330LHHX_DEV_NAME "_magn";
		break;
	case IIO_PRESSURE:
		iio_dev->name = ST_ASM330LHHX_DEV_NAME "_press";
		break;
	default:
		iio_dev->name = ST_ASM330LHHX_DEV_NAME "_ext";
		break;
	}
