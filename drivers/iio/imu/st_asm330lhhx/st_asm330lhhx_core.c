...
static struct st_asm330lhhx_power_mode_table {
	char *string_mode;
...
static const struct iio_chan_spec st_asm330lhhx_acc_channels[] = {
	...
	ST_ASM330LHHX_EVENT_CHANNEL(IIO_ACCEL, IIO_EV_TYPE_FIFO_FLUSH,
				  IIO_EV_DIR_EITHER),
...
static const struct iio_chan_spec st_asm330lhhx_gyro_channels[] = {
	...
	ST_ASM330LHHX_DATA_CHANNEL(IIO_ANGL_VEL, ST_ASM330LHHX_REG_OUTZ_L_G_ADDR,
				1, IIO_MOD_Z, 2, 16, 16, 's'),
	ST_ASM330LHHX_EVENT_CHANNEL(IIO_ANGL_VEL, IIO_EV_TYPE_FIFO_FLUSH,
				  IIO_EV_DIR_EITHER),
	IIO_CHAN_SOFT_TIMESTAMP(3),
};
...
static const struct iio_chan_spec st_asm330lhhx_temp_channels[] = {
	...
	ST_ASM330LHHX_EVENT_CHANNEL(IIO_TEMP, IIO_EV_TYPE_FIFO_FLUSH,
				  IIO_EV_DIR_EITHER),
...
static int st_asm330lhhx_get_odr_val(enum st_asm330lhhx_sensor_id id, int odr,
				   int uodr, struct st_asm330lhhx_odr *oe)
{
	int req_odr = ST_ASM330LHHX_ODR_EXPAND(odr, uodr);
	int sensor_odr;
	int i;
	...
	for (i = 0; i < st_asm330lhhx_odr_table[id].size; i++) {
		...
		if (sensor_odr >= req_odr) {
			oe->hz = st_asm330lhhx_odr_table[id].odr_avl[i].hz;
			oe->uhz = st_asm330lhhx_odr_table[id].odr_avl[i].uhz;
			oe->val = st_asm330lhhx_odr_table[id].odr_avl[i].val;
			       ...
			       int req_uodr)
{
	...
	err = st_asm330lhhx_get_odr_val(id, req_odr, req_uodr, &oe);
	if (err)
		return err;
	...
	return regmap_update_bits(hw->regmap,
				  ...
				  ST_ASM330LHHX_SHIFT_VAL(oe.val,
					st_asm330lhhx_odr_table[id].reg.mask));
}
				 ...
				 int val, int val2, long mask)
{
	...
	case IIO_CHAN_INFO_SCALE:
		...
		break;
	case IIO_CHAN_INFO_SAMP_FREQ: {
		struct st_asm330lhhx_odr oe = { 0 };

		err = st_asm330lhhx_get_odr_val(sensor->id, val, val2, &oe);
		if (!err) {
			sensor->odr = oe.hz;
			sensor->uodr = oe.uhz;
	...
	for (i = 0; i < ST_ASM330LHHX_SUSPEND_RESUME_REGS; i++) {
		if (st_asm330lhhx_suspend_resume[i].page != FUNC_CFG_ACCESS_0) {
			err = regmap_update_bits(hw->regmap,
					...
					ST_ASM330LHHX_SHIFT_VAL(st_asm330lhhx_suspend_resume[i].page,
					 ST_ASM330LHHX_REG_ACCESS_MASK));
			if (err < 0) {
				dev_err(hw->dev,
		...
		if (restore) {
			err = regmap_update_bits(hw->regmap,
					ST_ASM330LHHX_REG_FUNC_CFG_ACCESS_ADDR,
					ST_ASM330LHHX_REG_ACCESS_MASK,
					ST_ASM330LHHX_SHIFT_VAL(FUNC_CFG_ACCESS_0,
					 ST_ASM330LHHX_REG_ACCESS_MASK));
			if (err < 0) {
				dev_err(hw->dev,
					"failed to update %02x reg\n",
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (id == ST_ASM330LHHX_ID_ACC) {
		...
		st_asm330lhhx_set_full_scale(sensor, IIO_G_TO_M_S_2(122));
...
static int st_asm330lhhx_reset_device(struct st_asm330lhhx_hw *hw)
{
	...
	err = regmap_update_bits(hw->regmap, ST_ASM330LHHX_REG_CTRL3_C_ADDR,
				 ...
				 ST_ASM330LHHX_SHIFT_VAL(1, ST_ASM330LHHX_REG_SW_RESET_MASK));
	if (err < 0)
		return err;
	...
	err = regmap_update_bits(hw->regmap, ST_ASM330LHHX_REG_CTRL3_C_ADDR,
				 ...
				 ST_ASM330LHHX_SHIFT_VAL(1, ST_ASM330LHHX_REG_BOOT_MASK));
...
static int st_asm330lhhx_init_device(struct st_asm330lhhx_hw *hw)
{
	...
	err = regmap_update_bits(hw->regmap, ST_ASM330LHHX_REG_TAP_CFG0_ADDR,
				 ...
				 ST_ASM330LHHX_SHIFT_VAL(1, ST_ASM330LHHX_REG_LIR_MASK));
	if (err < 0)
		return err;
	...
	err = regmap_update_bits(hw->regmap, ST_ASM330LHHX_REG_CTRL3_C_ADDR,
				 ...
				 ST_ASM330LHHX_SHIFT_VAL(1, ST_ASM330LHHX_REG_BDU_MASK));
	if (err < 0)
		return err;

	err = regmap_update_bits(hw->regmap, ST_ASM330LHHX_REG_CTRL5_C_ADDR,
				 ST_ASM330LHHX_REG_ROUNDING_MASK,
				 ST_ASM330LHHX_SHIFT_VAL(3, ST_ASM330LHHX_REG_ROUNDING_MASK));
	if (err < 0)
		return err;
	...
	err = regmap_update_bits(hw->regmap, ST_ASM330LHHX_REG_CTRL4_C_ADDR,
				 ...
				 ST_ASM330LHHX_SHIFT_VAL(1, ST_ASM330LHHX_REG_DRDY_MASK));
	...
	return regmap_update_bits(hw->regmap, drdy_reg,
				  ...
				  ST_ASM330LHHX_SHIFT_VAL(1, ST_ASM330LHHX_REG_FIFO_TH_MASK));
}
						...
						enum st_asm330lhhx_sensor_id id)
{
	...
	case ST_ASM330LHHX_ID_ACC:
		...
		iio_dev->name = ST_ASM330LHHX_DEV_NAME "_accel";
	...
	case ST_ASM330LHHX_ID_TEMP:
		...
		iio_dev->name = ST_ASM330LHHX_DEV_NAME "_temp";
...
#ifdef CONFIG_IIO_ST_ASM330LHHX_EN_REGULATOR
...
#endif /* CONFIG_IIO_ST_ASM330LHHX_EN_REGULATOR */

int st_asm330lhhx_probe(struct device *dev, int irq, struct regmap *regmap)
{
...
iio_register_fail:
	for (i--; i >= ST_ASM330LHHX_ID_GYRO; i--) {
		if (!hw->iio_devs[i])
			continue;
...
iio_buffer_fail:
	if (hw->irq > 0)
		st_asm330lhhx_deallocate_buffers(hw);
...
out:
	...
	st_asm330lhhx_disable_regulator_action(hw);
...
int st_asm330lhhx_remove(struct device *dev)
{
	...
	return st_asm330lhhx_deallocate_buffers(hw);
}
...
static int __maybe_unused st_asm330lhhx_suspend(struct device *dev)
{
	...
	for (i = 0; i < ST_ASM330LHHX_ID_MAX; i++) {
		...
		if (!((hw->enable_mask & BIT(sensor->id)) &
		      ST_ASM330LHHX_WAKE_UP_SENSORS)) {
			err = st_asm330lhhx_set_odr(sensor, 0, 0);
			if (err < 0)
				return err;
		} else {
			err = st_asm330lhhx_set_odr(sensor,
						  ST_ASM330LHHX_MIN_ODR_IN_WAKEUP,
						  0);
			if (err < 0)
				return err;
		}
#else /* CONFIG_IIO_ST_ASM330LHHX_MAY_WAKEUP */
		/* do not disable sensors if requested by wake-up */
		err = st_asm330lhhx_set_odr(sensor, 0, 0);
		if (err < 0)
			return err;
...
#ifdef CONFIG_IIO_ST_ASM330LHHX_MAY_WAKEUP
	if (hw->enable_mask & ST_ASM330LHHX_WAKE_UP_SENSORS) {
