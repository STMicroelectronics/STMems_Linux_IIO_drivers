...
#define ST_ASM330LHHX_MLC_MAX_NUMBER		8
#define ST_ASM330LHHX_FSM_MAX_NUMBER		16
					 ...
					 bool enable)
{
	...
	if (sensor->status == ST_ASM330LHHX_MLC_ENABLED) {
		...
		/* enable fsm core */
		for (i = 0; i < ST_ASM330LHHX_FSM_MAX_NUMBER; i++) {
		...
		if (enable || i == ST_ASM330LHHX_FSM_MAX_NUMBER) {
			err = st_asm330lhhx_update_page_bits_locked(hw,
					ST_ASM330LHHX_EMB_FUNC_EN_B_ADDR,
				   ...
				   struct st_asm330lhhx_hw *hw)
{
	...
	while (i < fw->size) {
		...
		if (mlc_num >= ST_ASM330LHHX_MLC_MAX_NUMBER ||
		    fsm_num >= ST_ASM330LHHX_FSM_MAX_NUMBER)
			break;
	}
...
static void st_asm330lhhx_mlc_update(const struct firmware *fw, void *context)
{
	...
	if (ret > 0) {
		...
		for (i = 0; i < ST_ASM330LHHX_MLC_MAX_NUMBER; i++) {
			if (mlc_mask & BIT(i)) {
...
static int st_asm330lhhx_mlc_flush_all(struct st_asm330lhhx_hw *hw, bool remove)
{
	struct st_asm330lhhx_sensor *sensor_mlc;
	struct iio_dev *iio_dev;
	int ret = 0, id;
	int start = ST_ASM330LHHX_ID_MLC_0;

	if (remove)
		start = ST_ASM330LHHX_ID_MLC;

	for (id = start; id < ST_ASM330LHHX_ID_MAX; id++) {
				     ...
				     const char *buf, size_t size)
{
	...
	ret = st_asm330lhhx_mlc_flush_all(hw, 0);
...
static ssize_t st_asm330lhhx_mlc_odr(struct device *dev,
				   struct device_attribute *attr,
				   char *buf)
{
	struct st_asm330lhhx_sensor *sensor = iio_priv(dev_get_drvdata(dev));
...
static ssize_t st_asm330lhhx_mlc_x_odr(struct device *dev,
				     struct device_attribute *attr,
				     char *buf)
{
	struct st_asm330lhhx_sensor *sensor = iio_priv(dev_get_drvdata(dev));
...
static IIO_DEVICE_ATTR(mlc_x_odr, 0444,
		       st_asm330lhhx_mlc_x_odr, NULL, 0);
...
static const struct attribute_group st_asm330lhhx_mlc_x_event_attribute_group = {
	.attrs = st_asm330lhhx_mlc_x_event_attributes,
};
static const struct iio_info st_asm330lhhx_mlc_x_event_info = {
	.attrs = &st_asm330lhhx_mlc_x_event_attribute_group,
	.read_event_config = st_asm330lhhx_mlc_read_event_config,
	.write_event_config = st_asm330lhhx_mlc_write_event_config,
};
					      ...
					      enum st_asm330lhhx_sensor_id id)
{
	...
	case ST_ASM330LHHX_ID_MLC: {
		const struct iio_chan_spec st_asm330lhhx_mlc_channels[] = {
			ST_ASM330LHHX_EVENT_CHANNEL(IIO_ACTIVITY,
						  IIO_EV_TYPE_THRESH,
						  IIO_EV_DIR_RISING),
		};
	...
	case ST_ASM330LHHX_ID_MLC_7: {
		...
		scnprintf(sensor->name, sizeof(sensor->name),
			  ST_ASM330LHHX_DEV_NAME "_mlc_%d",
	...
	case ST_ASM330LHHX_ID_FSM_15: {
		...
		iio_dev->info = &st_asm330lhhx_mlc_x_event_info;
		scnprintf(sensor->name, sizeof(sensor->name),
			  ST_ASM330LHHX_DEV_NAME "_fsm_%d",
...
int st_asm330lhhx_mlc_remove(struct device *dev)
{
	...
	return st_asm330lhhx_mlc_flush_all(hw, 1);
}
