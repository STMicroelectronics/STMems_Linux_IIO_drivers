...
static const u8 mlcdata[] = {
	/*
	 * Machine Learning Core Tool v1.2.0.0 Beta, LSM6DSRX
	 * 6D position recognition
	 * source:
	 * https://github.com/STMicroelectronics/STMems_Machine_Learning_Core/blob/master/application_examples/lsm6dsrx/6D%20position%20recognition/lsm6dsrx_six_d_position.ucf
	 */
...
static int st_lsm6dsrx_mlc_flush_all(struct st_lsm6dsrx_hw *hw, bool remove)
{
	struct st_lsm6dsrx_sensor *sensor_mlc;
	struct iio_dev *iio_dev;
	int ret = 0, id;
	int start = ST_LSM6DSRX_ID_MLC_0;

	if (remove)
		start = ST_LSM6DSRX_ID_MLC;

	for (id = start; id < ST_LSM6DSRX_ID_MAX; id++) {
				     ...
				     const char *buf, size_t size)
{
	...
	ret = st_lsm6dsrx_mlc_flush_all(hw, 0);
					      ...
					      enum st_lsm6dsrx_sensor_id id)
{
	...
	case ST_LSM6DSRX_ID_MLC: {
		const struct iio_chan_spec st_lsm6dsrx_mlc_channels[] = {
			ST_LSM6DSRX_EVENT_CHANNEL(IIO_ACTIVITY,
						  IIO_EV_TYPE_THRESH,
						  IIO_EV_DIR_RISING),
		};
		...
		scnprintf(sensor->name, sizeof(sensor->name), ST_LSM6DSRX_DEV_NAME "_mlc");
		break;
	}
	...
	case ST_LSM6DSRX_ID_MLC_7: {
		...
		scnprintf(sensor->name, sizeof(sensor->name),
			  ST_LSM6DSRX_DEV_NAME "_mlc_%d", id - ST_LSM6DSRX_ID_MLC_0);
	...
	case ST_LSM6DSRX_ID_FSM_15: {
		...
		iio_dev->info = &st_lsm6dsrx_mlc_x_event_info;
		scnprintf(sensor->name, sizeof(sensor->name),
			  ST_LSM6DSRX_DEV_NAME "_fsm_%d", id - ST_LSM6DSRX_ID_FSM_0);
...
int st_lsm6dsrx_mlc_remove(struct device *dev)
{
	...
	return st_lsm6dsrx_mlc_flush_all(hw, 1);
}
