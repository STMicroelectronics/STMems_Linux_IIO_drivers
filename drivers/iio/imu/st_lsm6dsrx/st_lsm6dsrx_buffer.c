...
static inline int st_lsm6dsrx_reset_hwts(struct st_lsm6dsrx_hw *hw)
{
	...
	hw->ts = st_lsm6dsrx_get_time_ns();
	hw->ts_offset = hw->ts;
	hw->tsample = 0ull;

	return st_lsm6dsrx_write_locked(hw, ST_LSM6DSRX_REG_TIMESTAMP2_ADDR,
					data);
}
...
static irqreturn_t st_lsm6dsrx_handler_irq(int irq, void *private)
{
	...
	s64 ts = st_lsm6dsrx_get_time_ns();
...
int st_lsm6dsrx_buffers_setup(struct st_lsm6dsrx_hw *hw)
{
	...
	err = regmap_update_bits(hw->regmap,
				 ...
				 ST_LSM6DSRX_SHIFT_VAL(irq_active_low,
				  ST_LSM6DSRX_REG_H_LACTIVE_MASK));
	if (err < 0)
		return err;
	...
	err = regmap_update_bits(hw->regmap, ST_LSM6DSRX_REG_FIFO_CTRL4_ADDR,
