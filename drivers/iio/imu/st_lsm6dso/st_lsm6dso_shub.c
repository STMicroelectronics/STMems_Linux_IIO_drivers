...
#define ST_LSM6DSO_REG_SLAVE_NUMOP_MASK		0x07
...
static const struct st_lsm6dso_ext_dev_settings st_lsm6dso_ext_dev_table[] = {
	/* LIS2MDL */
	{
		...
		.ext_channels[2] = ST_LSM6DSO_DATA_CHANNEL(IIO_MAGN, 0x6c,
							   1, IIO_MOD_Z, 2,
							   16, 16, 's'),
		.ext_channels[3] = ST_LSM6DSO_EVENT_CHANNEL(IIO_MAGN,
...
/*
 * st_lsm6dso_shub_wait_complete - wait write trigger
 *
...
/*
 * st_lsm6dso_shub_read_reg - read from sensor hub bank register
 *
 * uses page_lock
 */
...
/*
 * st_lsm6dso_shub_write_reg - write to sensor hub bank register
 *
 * uses page_lock
 */
...
/*
 * st_lsm6dso_shub_master_enable - enable sensor hub interface
 *
 * uses page_lock
 */
				 ...
				 u8 *data, int len)
{
	...
	for (i = 0; i < len; i++) {
		...
		err = st_lsm6dso_shub_write_reg(hw, ST_LSM6DSO_REG_DATAWRITE_SLV0_ADDR,
						&data[i], 1);
		if (err < 0)
			return err;
