...
#define ST_LSM6DSO_FSM_BASE_ADDRESS			0x400
...
#define ST_LSM6DSO_REG_WR_MASK				0x60
...
#define ST_LSM6DSO_REG_ORIENTATION_90_MASK		BIT(7)
#define ST_LSM6DSO_REG_ORIENTATION_180_MASK		BIT(4)
#define ST_LSM6DSO_REG_ORIENTATION_270_MASK		BIT(6)

/* Finite State Machine ODR configuration */
#define ST_LSM6DSO_REG_EMB_FUNC_ODR_CFG_B_ADDR		0x5f
#define ST_LSM6DSO_REG_FSM_ODR_MASK			0x38
#define ST_LSM6DSO_FSM_ODR_12_5				0
#define ST_LSM6DSO_FSM_ODR_26				1
#define ST_LSM6DSO_FSM_ODR_52				2
#define ST_LSM6DSO_FSM_ODR_104				3
#define ST_LSM6DSO_FSM_ODR_208				4
#define ST_LSM6DSO_FSM_ODR_416				5

#define ST_LSM6DSO_REG_STEP_COUNTER_L_ADDR		0x62
#define ST_LSM6DSO_REG_EMB_FUNC_SRC_ADDR		0x64
#define ST_LSM6DSO_REG_PEDO_RST_STEP_MASK		BIT(7)
...
static const struct st_lsm6dso_fsm_sensor st_lsm6dso_fsm_sensor_list[] = {
	/* glance */
	{
		.id = ST_LSM6DSO_ID_GLANCE,
		.data = {
...
struct st_lsm6dso_fsm_fs {
	u32 gain;
	__le16 val;
};

static const struct st_lsm6dso_fsm_fs st_lsm6dso_fsm_fs_table[] = {
	{  ST_LSM6DSO_ACC_FS_2G_GAIN, 0x03ff },
	{  ST_LSM6DSO_ACC_FS_4G_GAIN, 0x07fe },
	{  ST_LSM6DSO_ACC_FS_8G_GAIN, 0x0bfe },
	{ ST_LSM6DSO_ACC_FS_16G_GAIN, 0x0ffe },
};

static inline
int st_lsm6dso_fsm_set_access(struct st_lsm6dso_hw *hw, bool enable)
{
	u8 val = enable ? 2 : 0;

	return __st_lsm6dso_write_with_mask(hw,
					    ST_LSM6DSO_PAGE_RW_ADDR,
					    ST_LSM6DSO_REG_WR_MASK,
					    val);
}
					   ...
					   u8 mask, u8 irq_mask, bool enable)
{
	...
	err = st_lsm6dso_set_page_access(hw, ST_LSM6DSO_REG_FUNC_CFG_MASK,
					 true);
	if (err < 0)
		goto unlock;
	...
	err = __st_lsm6dso_write_with_mask(hw, hw->embfunc_irq_reg, irq_mask,
					   enable);
reset_page:
				     ...
				     bool enable)
{
	...
	for (i = 0; i < ARRAY_SIZE(st_lsm6dso_fsm_sensor_list); i++)
		if (st_lsm6dso_fsm_sensor_list[i].id == sensor->id)
			break;

	if (i == ARRAY_SIZE(st_lsm6dso_fsm_sensor_list))
		return -EINVAL;
	...
	err = hw->tf->write(hw->dev, ST_LSM6DSO_REG_FSM_ENABLE_A_ADDR,
					 ...
					 bool enable)
{
	...
	case ST_LSM6DSO_ID_GLANCE:
		err = st_lsm6dso_fsm_set_enable(sensor, enable);
		break;
	default:
		err = -EINVAL;
		break;
...
int st_lsm6dso_reset_step_counter(struct iio_dev *iio_dev)
{
	...
	err = st_lsm6dso_step_counter_set_enable(sensor, true);
	if (err < 0)
		goto unlock_iio_dev;
	...
	do {
		prev_val = val;
		err = __st_lsm6dso_write_with_mask(hw,
					ST_LSM6DSO_REG_EMB_FUNC_SRC_ADDR,
					ST_LSM6DSO_REG_PEDO_RST_STEP_MASK, 1);
		if (err < 0)
			goto reset_page;
		...
		err = hw->tf->read(hw->dev,
				   ST_LSM6DSO_REG_STEP_COUNTER_L_ADDR,
				   sizeof(data), (u8 *)&data);
		if (err < 0)
			goto reset_page;
...
unlock_page:
	...
	err = st_lsm6dso_step_counter_set_enable(sensor, false);
unlock_iio_dev:
...
int st_lsm6dso_fsm_get_orientation(struct st_lsm6dso_hw *hw, u8 *out)
{
	...
	err = hw->tf->read(hw->dev, ST_LSM6DSO_REG_FSM_OUTS6_ADDR,
			   sizeof(data), &data);
	if (err < 0)
		goto reset_page;

	switch (data) {
	case ST_LSM6DSO_REG_ORIENTATION_0_MASK:
		*out = 0;
		break;
	case ST_LSM6DSO_REG_ORIENTATION_90_MASK:
		*out = 1;
		break;
	case ST_LSM6DSO_REG_ORIENTATION_180_MASK:
		*out = 2;
		break;
	case ST_LSM6DSO_REG_ORIENTATION_270_MASK:
		*out = 3;
		break;
	default:
		err = -EINVAL;
...
int st_lsm6dso_fsm_init(struct st_lsm6dso_hw *hw)
{
	...
	/* enable gesture rec */
	err = __st_lsm6dso_write_with_mask(hw,
					   ST_LSM6DSO_REG_EMB_FUNC_EN_B_ADDR,
					   ST_LSM6DSO_REG_FSM_EN_MASK,
					   1);
	if (err < 0)
		goto reset_page;
	...
	irq_mask = (1 << ARRAY_SIZE(st_lsm6dso_fsm_sensor_list)) - 1;
	...
	/* enable access */
	err = st_lsm6dso_fsm_set_access(hw, true);
	if (err < 0)
		return err;

	/* # of configured fsm */
	err = st_lsm6dso_fsm_write(hw, 0x17c, sizeof(nfsm), nfsm);
	if (err < 0)
		goto reset_access;

	err = st_lsm6dso_fsm_write(hw, 0x17e, sizeof(fsm_addr), (u8 *)
				   &fsm_addr);
	if (err < 0)
		goto reset_access;
	...
	/* configure fsm */
	for (i = 0; i < ARRAY_SIZE(st_lsm6dso_fsm_sensor_list); i++) {
		err = st_lsm6dso_fsm_write(hw, fsm_addr,
					st_lsm6dso_fsm_sensor_list[i].len,
					st_lsm6dso_fsm_sensor_list[i].data);
		if (err < 0)
			goto reset_access;

		fsm_addr += st_lsm6dso_fsm_sensor_list[i].len;
	}

reset_access:
	st_lsm6dso_fsm_set_access(hw, false);
