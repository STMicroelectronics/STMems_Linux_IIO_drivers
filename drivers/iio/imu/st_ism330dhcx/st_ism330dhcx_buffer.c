 ...
 */
static inline s64 st_ism330dhcx_get_time_ns(void)
{
	struct timespec ts;
...
inline int st_ism330dhcx_reset_hwts(struct st_ism330dhcx_hw *hw)
{
	...
	hw->ts = st_ism330dhcx_get_time_ns();
	hw->ts_offset = hw->ts;
	hw->val_ts_old = 0;
	hw->hw_ts_high = 0;
	hw->tsample = 0ull;
...
static irqreturn_t st_ism330dhcx_handler_irq(int irq, void *private)
{
	...
	s64 ts = st_ism330dhcx_get_time_ns();
...
static int st_ism330dhcx_fifo_preenable(struct iio_dev *iio_dev)
{
	...
	err = st_ism330dhcx_update_fifo(iio_dev, true);
	if (err < 0)
		return err;
...
static int st_ism330dhcx_buffer_preenable(struct iio_dev *iio_dev)
{
	...
	err = st_ism330dhcx_embfunc_sensor_set_enable(iio_priv(iio_dev), true);
	if (err < 0)
		return err;
...
int st_ism330dhcx_deallocate_buffers(struct st_ism330dhcx_hw *hw)
{
	struct iio_dev *iio_dev = hw->iio_devs[ST_ISM330DHCX_ID_ORIENTATION];
	struct st_ism330dhcx_sensor *sensor = iio_priv(iio_dev);
	int i;
